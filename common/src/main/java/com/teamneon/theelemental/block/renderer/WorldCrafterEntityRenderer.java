...
public class WorldCrafterEntityRenderer implements BlockEntityRenderer<WorldCrafterEntity, WorldCrafterEntityRenderState> {
    private final ItemModelResolver itemModelManager;

    public WorldCrafterEntityRenderer(BlockEntityRendererProvider.Context context) {
        this.itemModelManager = context.itemModelResolver();
    }

    @Override
    public WorldCrafterEntityRenderState createRenderState() {
        return new WorldCrafterEntityRenderState();
    }

    @Override
    public void extractRenderState(WorldCrafterEntity blockEntity, WorldCrafterEntityRenderState state, float partialTick, Vec3 cameraPosition, ModelFeatureRenderer.@Nullable CrumblingOverlay breakProgress) {
    ...
    @Override
    public void submit(WorldCrafterEntityRenderState state, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState cameraRenderState) {
